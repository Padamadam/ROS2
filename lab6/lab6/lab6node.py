...
class lab6node(Node):
    ...
    def listener_callback(self, msg):
        if self.is_first == False:
            ...
            for i in range(len(msg.marker_ids)):
                if msg.marker_ids[i] == 14:
                    self.first_move_x = msg.poses[i].position.x
                    self.first_move_y = msg.poses[i].position.y
                    self.first_move_z = msg.poses[i].position.z
                if msg.marker_ids[i]== 1:
                    self.end_move_x = msg.poses[i].position.x
                    self.end_move_y = msg.poses[i].position.y
                    self.end_move_z = msg.poses[i].position.z
            self.go_to()


    def go_to(self):
        if self.state == 0:
            point = [150.0, 0.0, 100.0, 0.0]
            self.get_logger().info('udalosieieiiskjsdkfjgdihtrjnhtoijd')
            self.get_logger().info(str(self.state))
            self.move(point)
        elif self.state == 1:
            # point = [self.first_move_x, self.first_move_y, self.first_move_z, 0.0]
            point = [100.0, 0.0, 100.0, 0.0]
            
            self.move(point)
            self.get_logger().info('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa')
        elif self.state == 2:            # point = [self.first_move_x, self.first_move_y, self.first_move_z, 0.0]
            point = [100.0, 0.0, 100.0, 0.0]
    ...
    def move(self, point):
        first_goal = PointToPoint.Goal()
        first_goal.motion_type = 1
        first_goal.target_pose = point
        first_goal.velocity_ratio = 0.5
        first_goal.acceleration_ratio = 0.5
        self.client.wait_for_server()

        future = self.client.send_goal_async(first_goal)
    ...
    def execute_result_callback(self, future):
        self.get_logger().info('ccccccccccccccccccccccccccccc')
        # self.go_to()
        self.state += 1
        self.get_logger().info('bbbbbbbbbbbbbbbbbbbbbbbbbbbbb')
